...
public class ItemManage extends RpgPlayer{

    public ItemManage(IGameEngine gameEngine) {
        super(gameEngine);
    }
    ...
    public boolean pickUpItem(Item item) {
        int weight = super.inventoryManage.calculateInventoryWeight();
        if (weight + item.getWeight() > carryingCapacity)
            return false;

        if (item.isUnique() && super.inventoryManage.checkIfItemExistsInInventory(item))
            return false;
        ...
        super.combatManage.calculateStats();
